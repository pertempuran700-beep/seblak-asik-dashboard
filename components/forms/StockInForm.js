...
export default function StockInForm({ products, vendors, onSuccess, onClose }) {
  ...
  const finalProductId = form.productId || filteredProducts[0]?.product_id;
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="bg-surface2 p-3 rounded-card border border-white/[0.08] mb-2">
        ...
        <p className="text-xs text-textmuted mt-2">
          *Atau pilih kategori & produk manual di bawah jika tidak hafal kode.
        </p>
      </div>
      ...
      <Select
        label="2. Nama Produk"
        options={filteredProducts.map((p) => ({ value: p.product_id, label: `${p.name} (${p.kode})` }))}
