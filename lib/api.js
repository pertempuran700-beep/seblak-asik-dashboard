...
export const api = {
  ...
  getDashboardSummary: (startDate, endDate) => callApi('getDashboardSummary', { startDate, endDate }), // <-- Router Baru
  ...
  // REVISI BARU: Menghubungkan fungsi aksi persetujuan izin instan dari lonceng ke backend
  approveLeave: (leaveId, approved, notes) => callApi('approveLeave', { leaveId, approved, notes }),
};
// Tambahkan ini di dalam object `api` Anda
  submitPerformanceReview: async (employeeId, period, scores) => {
    return postAction('submitPerformanceReview', { employeeId, period, scores });
  },
  getDailyBonusLog: async (period) => {
    return postAction('getDailyBonusLog', { period });
  },
