...
export const api = {
  ...
  addStockIn: (productId, vendorId, qtyBeli, hargaBeli, paymentStatus, dueDate, invoiceNo, transactionDate) => callApi('addStockIn', { productId, vendorId, qtyBeli, hargaBeli, paymentStatus, dueDate, invoiceNo, transactionDate }),
  ...
  getPayslip: (employeeId, month, year) => callApi('getPayslip', { employeeId, month, year }),
  ...
  addWasteProduct: (productId, quantity, notes, transactionDate) => callApi('addWasteProduct', { productId, quantity, notes, transactionDate }),
  getWasteReport: (month, year) => callApi('getWasteReport', { month, year }),
  getStockMovementReport: (startDate, endDate) => callApi('getStockMovementReport', { startDate, endDate }),
  listOtherCostItems: () => callApi('listOtherCostItems'),
  createOtherCostItem: (itemName, category, description) => callApi('createOtherCostItem', { itemName, category, description }),
  updateOtherCostItem: (itemId, updates) => callApi('updateOtherCostItem', { itemId, updates }),
    createOtherCostExpense: (itemId, amount, notes, transactionDate) => callApi('createOtherCostExpense', { itemId, amount, notes, transactionDate }),
  getOtherCostsReport: (month, year) => callApi('getOtherCostsReport', { month, year }),
  getProductBudgetReport: (startDate, endDate) => callApi('getProductBudgetReport', { startDate, endDate }),
  getCashOutflowDashboard: (month, year) => callApi('getCashOutflowDashboard', { month, year }),
  createMarketingCost: (period, amount, notes) => callApi('createMarketingCost', { period, amount, notes }),
  listMarketingCosts: () => callApi('listMarketingCosts'),
  getMarketingDashboard: (month, year) => callApi('getMarketingDashboard', { month, year }),
  getTodayAttendanceRekap: (date) => callApi('getTodayAttendanceRekap', { date }),
};
